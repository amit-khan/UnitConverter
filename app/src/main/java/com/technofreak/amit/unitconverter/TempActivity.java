...
public class TempActivity extends AppCompatActivity implements AdapterView.OnItemSelectedListener {
    ...
    private final String[] length = {"Celsius (°C)", "Fahrenheit (°F)", "Kelvin (°K)"};
    private EditText editText;
    private ListView listView;
    double value;
    int position;
    Spanned[] ans = new Spanned[2]; //chng

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.layout_length);
        setTitle("Temperature");  //chng
            ...
            if (v.length() > 0) {
                ...
                ArrayAdapter<Spanned> listAdapter = new ArrayAdapter<Spanned>(this, R.layout.row_unit, ans) {
                    ...
                    public View getView(int position, View convertView, @NonNull ViewGroup parent) {
                        ...
                        if (convertView == null) {
                            view = mInflater.inflate(R.layout.row_unit, parent, false);
                        } else {
                            view = convertView;
                        }
    ...
    private void convert(double value) {
        for (int i = 0, j = 0; i < 2; i++, j++) {
            if (j == position) j++;
            double d = 0;
            if (position == 0 && j == 1) {  //C to F
                d = (value * 1.8) + 32;
            } else if (position == 0 && j == 2) {  //C to K
                d = value + 273.15;
            } else if (position == 1 && j == 0) {  //F to C
                d = (value - 32) * (5.0 / 9);
            } else if (position == 1 && j == 2) {  //F to K
                d = (5.0 / 9) * (value - 32) + 273.15;
            } else if (position == 2 && j == 0) {  //K to C
                d = value - 273.15;
            } else if (position == 2 && j == 1) {  //K to F
                d = (9.0 / 5) * (value - 273.15) + 32;
            }
